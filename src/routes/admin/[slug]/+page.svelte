...
<main class='flex flex-col'>
  ...
  <div class='flex flex-col ml-4 mb-8 w-full bg-white rounded p-4'>
    ...
    <p class='text-xl'>This is the administrative access point. If you are not an authorized administrator, please leave now.</p>
  </div>
  <div class='flex flex-col ml-4 mb-8 w-full bg-white rounded p-4'>
    ...
    <select on:change={route_to_database} class="block appearance-none w-full mb-2 bg-white border border-gray-300 hover:border-gray-400 px-4 py-2 pr-8 rounded cursor-pointer shadow leading-tight focus:outline-none focus:shadow-outline-blue w-56">
        ...
        <table class='border-collapse w-full'>
          ...
          <tbody>
            ...
            <tr>
              ...
              {/each}
              <td class='border-solid border-2 border-gray-200 p-2 cursor-pointer text-center text-m m-10'>
                <form action="?/Save_Row" method="POST">
                  <button class="transition-all duration-200 p-3 rounded bg-gray-200 text-xl hover:bg-gray-300" name={i.toString()} on:click={(event)=>stringify(event.currentTarget.getAttribute("name"))}>
                    Save
                  </button>
              ...
              <td class='border-solid border-2 border-gray-200 p-2 cursor-pointer text-center text-m m-10'>
                <form action="?/Remove_Row" method="POST">
                  <button class="transition-all duration-200 p-3 rounded bg-violet-200 text-xl hover:bg-violet-300">
                    Delete
                  </button>
  ...
  <div class='flex flex-col ml-4 w-full bg-white rounded p-4'>
    ...
    <button class="transition-all duration-200 p-3 rounded bg-violet-200 text-xl hover:bg-violet-300" on:click={toggle_popup_visibility}> Archive Presentations</button>
  </div>
</main>
