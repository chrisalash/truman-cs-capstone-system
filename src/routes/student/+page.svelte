            ...
            <tr>
              {#each presentations as presentation}
                {#if presentation.time_start[i] != undefined}
                  ...
                  {:else if presentation.username[i] == usernameValue}
                    <!-- Display a button to remove the user if a slot has their username -->
                    <td
                      style="height: 74px; min-width: 600px"
                      class="border-solid border-2 border-gray-200 hover:bg-gray-300 p-2 cursor-pointer font-bold text-center text-xl m-10 relative"
                    >
                  ...
                  {:else}
                    ...
                    <td
                      style="height: 74px; min-width: 600px"
                      class="border-solid border-2 border-gray-200 p-2 text-center text-xl m-10"
                    >
                      {presentation.time_start[i]}-{presentation.time_end[i]}<br />
                    </td>
                  {/if}
