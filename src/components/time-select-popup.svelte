<script lang="ts">
  ...
  function interval_add() {
    ...
    while (
      editedHours < endHours &&
      !(editedHours == endHours && editedMinutes + interval > endMinutes)
    ) {
      // Make sure the hours and minutes are within valid ranges
      let start = `${editedHours.toString().padStart(2, '0')}:${editedMinutes
        .toString()
        .padStart(2, '0')}`;
      ...
      let end = `${editedHours.toString().padStart(2, '0')}:${editedMinutes
        .toString()
        .padStart(2, '0')}`;
      ...
      <div class="flex flex-col justify-center items-center">
        ...
        <div class="flex flex-row mt-4 ">
          ...
          <div class="flex flex-col">
            ...
            <select
              class="transition-all duration-200 border-solid border border-gray-200 cursor-pointer h-10 w-40 text-2xl hover:bg-gray-300"
              bind:value={interval}
            >
